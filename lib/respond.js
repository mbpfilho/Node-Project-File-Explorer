...
//following ia function passed to createServer used to create the server
    ...
    //can we find something in fullStaticPath?
        ...
        try{
            ...
            if(stats.isDirectory()){
                //get content from the template server (index.html)
                ...
                //fill the remplate data with: the page title, breadcrumb and table rows (main_content)
            ...
            // stats.isDirectory().toString();

            //it's not a directory neigther a file
                //send: 401: Access denied!
                if(!stats.isFile()){
                    response.statusCode=401;
                    response.write('401: Acess denied!');
            ...
            //it's a file
                ...
                getMimeType(fileDetails.extname).then(mime=>{
                    ...
                    let statusCode=200;
                    //set "Content-Type" for all fiel types
                    head['Content-Type']=mime;

                    
                //get the file size and add it to the response hearder
                //pdf file? -> display in
                ...
                //all other files stream in a normal
                    ...
                    //stream chunks of data to response object
                    response.writeHead(statusCode,head);
